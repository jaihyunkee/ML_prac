...
class KNN():
    ...
    def manhattan_distance(self, x1: np.ndarray, x2: np.ndarray) -> float:
        ...
        # >> YOUR CODE HERE
        temp = np.absolute(x1 - x2)
        manh_dist = np.sum(temp)
        # END OF YOUR CODE <<
    ...
    def get_distances_to_other_samples(self, x: np.ndarray) -> np.ndarray:
        ...
        for i in range(len(self.X_train)):
            a = self.manhattan_distance(self.X_train[i], x)
            dist[i] = a
        # END OF YOUR CODE <<
        dist = np.array(dist)
        return dist
    ...
    def vote(self, k_nearest: np.ndarray) -> int:
        ...
        # >> YOUR CODE HERE
        temp = np.zeros(len(k_nearest))
        temp = temp.astype(int)
        for i in range(len(k_nearest)):
            temp[i] = self.y_train[int(k_nearest[i])]
        temp = np.array(temp)
        bin = np.bincount(temp)
        y_pred = np.argmax(bin)
        # END OF YOUR CODE <<
    ...
    def get_k_nearest_neighbors(self, dist: np.ndarray) -> np.ndarray:
        ...
        k_nearest = np.zeros(self.k)
        for i in range(self.k):
            k_nearest[i] = sortIndices[i]
