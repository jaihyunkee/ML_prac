...
class UncertaintyScorer(ABC):
	...
	def split_on_best(self, X, y, exclude=set()):
		...
		## >>> YOUR CODE HERE >>>
		unique = set(X[:, best_feature])
		for i in unique:
			a, b = self.subset_data(X, y, best_feature, i)
			subsets[i] = (a, b)
		## <<< END OF YOUR CODE <<<
...
class InfoGain(UncertaintyScorer):
	...
	def score(self, y):
		...
		"""
		## >>> YOUR CODE HERE >>>
		diction = self.compute_class_probs(y)
		prob = list(diction.values())
		entropy = float(0)
		for i in prob:
			if i == 0:
				entropy += 0
	...
	def compute_gain(self, X, y, j):
		...
		"""
		## >>> YOUR CODE HERE >>>
		n_of_feature = set(X[:, j])
		total = self.score(y)
		summ = 0
		for i in n_of_feature:
			X_subset, y_subset = self.subset_data(X, y, j, i)
			entropy = self.score(y_subset)
			summ += entropy * (len(y_subset)/len(X))
		info_gain = total - summ
		## <<< END OF YOUR CODE <<<
		return info_gain