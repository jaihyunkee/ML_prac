...
def find_best_param(X: np.ndarray,
    ...
    for val in param_vals:
        ...
        train_accs, val_accs = cv.cross_val_score(clf, X, y)
        train_accs_cv[val] = np.mean(train_accs)
        val_accs_cv[val] = np.mean(val_accs)

        if np.mean(val_accs) > best_acc:
            best_param = val
            best_acc = np.mean(val_accs)
