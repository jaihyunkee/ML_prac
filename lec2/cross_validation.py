...
class CrossValidation(object):
    ...
    def create_folds(self,
        ...
        # >> YOUR CODE HERE
        n = len(X)
        n_element_in_each_fold = n // self.k
        longer = n % self.k

        folds = []
        start_idx = 0
        for i in range(self.k):
            end_idx = start_idx + n_element_in_each_fold
            if longer != 0 and i == 0:
                end_idx += longer
    ...
    def train_valid_split(self, folds: list, use_as_valid: int = 0) -> tuple:
        ...
        X_val, y_val = folds[use_as_valid]
        X_train, y_train = [], []

        # >>> YOUR CODE HERE
        for i, fold in enumerate(folds):
            if i != use_as_valid:
                X_train.append(fold[0])
                y_train.append(fold[1])
        # << END OF YOUR CODE

        return (np.concatenate(X_train), np.concatenate(y_train)), (X_val, y_val)
