...
class LogisticRegression(object):
    ...
    def gradient(self, X_train: np.ndarray, y_train: np.ndarray) -> np.ndarray:
        ...
        # >> YOUR CODE HERE
        partial = self.sigmoid(np.dot(X_train, self.weights) + self.bias) - y_train
        grad_w = np.dot(X_train.T, partial) / len(y_train)
        grad_b = sum(partial) / len(y_train)
        # << END OF YOUR CODE
    ...
    def logistic_loss(self, X_train: np.ndarray, y_train: np.ndarray) -> float:
        ...
        logistic_loss = np.mean(-y_train * np.log(y_pred) - (1 - y_train) * np.log(1 - y_pred))
        # << END OF YOUR CODE
    ...
    def train(self,
        ...
        for epoch in range(max_epochs):
            ...
            # >> YOUR CODE HERE
            grad_w, grad_b = self.train_one_epoch(X_train=X_train, y_train=y_train, learning_rate=lr)
            train_loss = self.logistic_loss(X_train, y_train)
            valid_loss = self.logistic_loss(X_valid, y_valid)
            train_acc = accuracy(y_train, y_pred=self.predict(X_train))
            valid_acc = accuracy(y_valid, y_pred=self.predict(X_valid))
            # << END OF YOUR CODE
...
def plot_logistic_regression_curve(X_train: np.ndarray,
    ...
    for _ in range(max_epochs):
        ...
        ## >>> YOUR CODE HERE
        lr_classifier.train_one_epoch(X_train, y_train, lr)
...
def evaluate_lr():
    ...
    plot_logistic_regression_curve(
        X_train, y_train, X_valid, y_valid, max_epochs=12000, lr=0.001)
