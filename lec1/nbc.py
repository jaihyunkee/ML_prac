...
class NaiveBayesClassifier:
	...
	def compute_class_probability(self, y_train):
		...
		for label in unique:
			...
			class_probabilities[label] = (count + self.alpha) / (n + len(unique) * self.alpha)
		## <<< END OF YOUR CODE <<<
	...
	def compute_feature_probability(self, Xj_train, y_train):
		...
		## >>> YOUR CODE HERE >>>
		unique_X = np.unique(Xj_train)
		for i in range(2):
			n = np.count_nonzero(y_train == i)
			divide = n + self.alpha * len(unique_X)
			for j in unique_X:
				temp = Xj_train[y_train == i]
				count = np.count_nonzero(temp == j)
				feature_probabilities[(j, i)] = (count + self.alpha) / divide
		## <<< END OF YOUR CODE <<<
	...
	def predict_probabilities(self, X_test):
		...
		for i in range(len(X_test)):
			for j in range(len(self.class_labels)):
				probs[i, j] = self.class_probs[self.class_labels[j]]
				for x_col in range(self.d):
					x = X_test[i, x_col]
					if (x, self.class_labels[j]) in self.feature_probs[x_col].keys():
						probs[i, j] *= self.feature_probs[x_col][(x, self.class_labels[j])]
	...
	def evaluate(self, y_test, probs):
		...
		zero_one_loss = sum / len(y_test)
		squared_loss = np.sum((y_test - y_pred) ** 2) * (1 / len(y_test))
	...
	def plot_nbc_curve(nbc, X, y):
		...
		for p in training_percentages:
			X_train, X_test, y_train, y_test = my_train_test_split(X=X, y=y, test_pct=1 - p)
			nbc.fit(X_train, y_train)

			probs = nbc.predict_probabilities(X_train)
			y_pred = nbc.predict(probs)
			training_accuracies.append(accuracy(y_train, y_pred))
