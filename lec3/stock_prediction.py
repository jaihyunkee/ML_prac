...
def pre_process(file_path: str) -> dict:
    ...
    for stock in stocks:
        ...
        stock_dict[stock] = stock_df.reset_index(drop=True)
    # <<< END YOUR CODE
...
def split_stock(stock_info: pd.DataFrame) -> tuple:
    ...
    n_row = len(stock_info)
    n_train_sample = int(n_row * 0.7)
    n_val_sample = int(n_row - n_train_sample)
    for i in range(n_row - 5):
        x_sample = stock_info.iloc[i:i + 5, :]['Close'].values.tolist()
        y_sample = stock_info.iloc[i + 5, :]['Close']
        if i < n_train_sample:
            x_train_stock.append(x_sample)
            y_train_stock.append(y_sample)
        else:
            x_val_stock.append(x_sample)
            y_val_stock.append(y_sample)
    # <<< END YOUR CODE
    x_train_stock = np.array(x_train_stock)
    y_train_stock = np.array(y_train_stock)
    x_val_stock = np.array(x_val_stock)
    y_val_stock = np.array(y_val_stock)
...
def get_train_valid(stock_dict: dict) -> tuple:
    ...
    for stock in stock_dict:
        ...
        x_val_list.append(x_val_stock)
        y_val_list.append(y_val_stock)
...
def my_NLLloss(pred: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    ...
    var = torch.exp(pred_risk)
    nll_loss = torch.mean(0.5 * torch.log(2 * torch.pi * var) + (y - pred_price) ** 2 / (2 * var))
    # <<< END YOUR CODE
    return nll_loss
...
def train(data: tuple, max_epochs: int = 200, seed=12345) -> tuple:
    ...
    data = (torch.from_numpy(d).float().to(device) for d in data)
    ...
    for epoch in iterator: # DO NOT CHANGE THIS LINE
        ...
        with torch.no_grad():
            ...
            val_loss += my_NLLloss(y_val_pred, y_val)
        # <<< END YOUR CODE
